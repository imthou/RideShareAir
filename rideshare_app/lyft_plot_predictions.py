...
model1_file = 'data/lyft_rf_model_forecast.csv'
# model2_file = 'data/model2_wo_surgemulti_forecast.csv'
model3_file = 'data/ridgecv_lyft_forecast.csv'
model4_file = 'data/lyft_arima_forecast.csv'
model5_file = 'data/lyft_xgboost_model_forecast.csv'
...
def mongo_query():
    ...
    docs = collection.find({'record_time':{'$gte':start_date,
                                    '$lte':end_date}},
                            {'record_time': 1, 'city':1, 'cost':1, '_id':0})
    data = []
    print "organizing lyft data..."
    length = docs.count()
    i = 0
    for doc in docs:
        df = pd.DataFrame(doc['cost'])
        df['avg_price_est'] = (df['estimated_cost_cents_max'] + df['estimated_cost_cents_min']) / 200.
        df['record_time'] = pd.to_datetime(doc['record_time'], unit='s') - pd.Timedelta(hours=7)
        df['display_name'] = df['ride_type'].apply(lambda x: x.lower())
        df['city'] = doc['city']
        data.append(df[['record_time','city','display_name','avg_price_est']])
        if i % 50 == 0:
            print "progress: {}, {:.2f}%".format(i, float(i)/length * 100.0)
        i += 1
    print "finished lyft collecting docs"
    df = pd.concat(data)
    ...
    forecast_name = "data/lyft_live_forecast_data.csv"
...
def create_plots(model1, model3, model4, model5, live_data, city, display_name):
    ...
    """
    if city != 'chicago':
        model1 = model1.query("city_{} == 1 and ride_type_{} == 1".format(city, display_name))
        # model2 = model2.query("city_{} == 1 and display_name_{} == 1".format(city, display_name))
        model3 = model3.query("city_{} == 1 and ride_type_{} == 1".format(city, display_name))
        model4 = model4.query("city_{} == 1 and ride_type_{} == 1".format(city, display_name))
        model5 = model5.query("city_{} == 1 and ride_type_{} == 1".format(city, display_name))
    else:
        model1 = model1.query("city_denver == 0 and city_seattle == 0 and city_sf == 0 and city_ny == 0 and ride_type_{} == 1".format(display_name))
        # model2 = model2.query("city_denver == 0 and city_seattle == 0 and city_sf == 0 and city_ny == 0 and display_name_{} == 1".format(display_name))
        model3 = model3.query("city_denver == 0 and city_seattle == 0 and city_sf == 0 and city_ny == 0 and ride_type_{} == 1".format(display_name))
        model4 = model4.query("city_denver == 0 and city_seattle == 0 and city_sf == 0 and city_ny == 0 and ride_type_{} == 1".format(display_name))
        model5 = model5.query("city_denver == 0 and city_seattle == 0 and city_sf == 0 and city_ny == 0 and ride_type_{} == 1".format(display_name))
...
def build_plots():
    ...
    for city in ['denver','ny','chicago','seattle','sf']:
        for cartype in ['lyft_line','lyft','lyft_plus']:
...
def callback(session, new_pts):
    ...
    """
    print "collecting lyft data again: {}", pd.to_datetime(time.time(), unit='s')
    live_data = mongo_query()
    print live_data.tail()
...
if __name__ == '__main__':
    ...
    with open('templates/lyft_model.html', 'w+') as f:
        f.write(html)
    ...
    while True:
        time.sleep(3600)
        print "pulling lyft data from Mongo EC2 again"
