...
class UberAPIRequest(object):
    ...
    def run(self):
        ...
        """
        d = self.cities_addresses()

        self.multi_threading(d, server_token, product_url, time_estimate_url, price_estimate_url, collection)

        print 'done at {0}. count of docs in mongo: {1}.'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())), collection.count())

        client.close()

    def multi_threading(self, cities_dct, server_token, product_url, time_url, price_url, collection):
        """
    ...
    def run_thread(self, city, addresses, server_token, product_url, time_url, price_url, collection):
        ...
        product_para, time_para, price_para = self.parameters(start, stop, server_token)

        # make requests to uber to obtain json documents of product, time, and price data for each city
        product_data, time_data, price_data = self.make_requests(product_url, product_para, time_url, time_para, price_url, price_para)
    ...
    def parameters(self, start, stop, server_token):
        """
        ...
        return product_para, time_estimate_para, price_estimate_para

    def make_requests(self, product_url, product_para, time_url, time_para, price_url, price_para):
        """
        ...
        product_data = requests.get(product_url, params=product_para)
        time_data = requests.get(time_url, params=time_para)
        price_data = requests.get(price_url, params=price_para)
        for r in [product_data,time_data,price_data]:
            if r.status_code != 200:
                print 'WARNING:', r.status_code
        return product_data.json(), time_data.json(), price_data.json()
